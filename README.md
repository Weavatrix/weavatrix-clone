...
The [Node/Bun benchmark report](node/benchmark/RESULTS.md) is generated by the
[weavatrix-benchmarks](https://github.com/Weavatrix/weavatrix-benchmarks)
harness, which compares against `jscpd` 4.3.0 on a 720-file corpus with planted
ground truth. Both tools found all 60 byte-identical pairs and reported no pair
that was never planted; Weavatrix also found all 60 renamed pairs, which
`jscpd` does not detect, and across three independent runs was **30.7x** faster
on Node and **17.9x** on Bun.
